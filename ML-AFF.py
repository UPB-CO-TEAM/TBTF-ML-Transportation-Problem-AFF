...
with st.sidebar:
    ...
    # Optional: real Claude API key for dynamic content
    with st.expander(t("⚙️ Opțional: cheie Claude API (pentru AI insights live)",
                       "⚙️ Optional: Claude API key (for live AI insights)")):
        api_key = st.text_input(
            t("Cheia ta Anthropic API", "Your Anthropic API key"),
            type="password",
            help=t("Lăsa gol pentru dicționarul pre-generat. Dacă pui o cheie, butoanele AI vor face apeluri live.",
                   "Leave empty to use the pre-generated dictionary. With a key, AI buttons will make live calls."),
        )
...
with tab3:
    ...
    with st.expander(t("✂️ Tăietura minimă (T)", "✂️ Minimum cut (T)")):
...
with tab4:
    ...
    # ========================================================================
    # CURBA ANIMATĂ — Simulare cronologică a crizei
    # ========================================================================
    st.markdown('<div class="section-spacer"></div>', unsafe_allow_html=True)
    st.markdown(f"##### 🎬 {t('Simulare cronologică — evoluția crizei lună de lună', 'Chronological simulation — month-by-month crisis evolution')}")
    st.markdown(t(
        f"Capacitatea NVIDIA scade liniar de la 500 (ian 2026) la {cap_nvidia} (dec 2026). "
        "Apasă **▶ Play** pentru a urmări cum se acumulează deficitul cumulat și cum oscilează costul PTE.",
        f"NVIDIA capacity decreases linearly from 500 (Jan 2026) to {cap_nvidia} (Dec 2026). "
        "Press **▶ Play** to watch how cumulative deficit accumulates and PTE cost oscillates."
    ))

    months_ro = ["Ian", "Feb", "Mar", "Apr", "Mai", "Iun", "Iul", "Aug", "Sep", "Oct", "Noi", "Dec"]
    months_en = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"]
    months_label = months_en if st.session_state.lang == "en" else months_ro

    cap_traj = np.linspace(500, cap_nvidia, 12)
    cost_traj, flux_traj, deficit_traj = [], [], []
    for c_n in cap_traj:
        A_m = [c_n, cap_amd, cap_intel]
        C_m, A_me, B_me, _, _ = echilibreaza(C_MATRIX, A_m, necesar)
        _, fm = rezolva_pte_linprog(C_m, A_me, B_me)
        g_m = [[0] * N for _ in range(N)]
        g_m[0][1] = c_n; g_m[0][2] = cap_amd; g_m[0][3] = cap_intel
        for s in [1, 2, 3]:
            for r in [4, 5, 6, 7, 8]:
                g_m[s][r] = 10 ** 5
        for idx, r in enumerate([4, 5, 6, 7, 8]):
            g_m[r][9] = cerere_2026[idx]
        mfm, _, _, _ = ford_fulkerson(g_m, 0, 9)
        cost_traj.append(fm)
        flux_traj.append(mfm)
        deficit_traj.append(max(0, cerere_totala - mfm))

    deficit_cum = np.cumsum(deficit_traj)

    # Animație frame-by-frame: linia se desenează progresiv
    frames = []
    for k in range(1, 13):
        frames.append(go.Frame(
            data=[
                go.Scatter(x=months_label[:k], y=deficit_cum[:k],
                           mode="lines+markers", name=t("Deficit cumulat", "Cumulative deficit"),
                           line=dict(color=ORANGE, width=4), marker=dict(size=12),
                           fill="tozeroy", fillcolor="rgba(251,140,0,0.18)"),
                go.Scatter(x=months_label[:k], y=cost_traj[:k],
                           mode="lines+markers", name=t("Cost PTE", "PTE cost"),
            ...
            layout=dict(
                annotations=[dict(
                    x=months_label[k - 1], y=deficit_cum[k - 1],
                    text=f"<b>{int(deficit_cum[k - 1])} u.p.</b>",
                    showarrow=True, arrowhead=2, arrowcolor=ORANGE, font=dict(color=ORANGE, size=14),
                )]
            ),
        ))
    ...
    fig_anim = go.Figure(
        data=[
            go.Scatter(x=[months_label[0]], y=[deficit_cum[0]],
                       mode="lines+markers", name=t("Deficit cumulat (u.p.)", "Cumulative deficit (u.p.)"),
                       line=dict(color=ORANGE, width=4), marker=dict(size=12),
                       fill="tozeroy", fillcolor="rgba(251,140,0,0.18)"),
            go.Scatter(x=[months_label[0]], y=[cost_traj[0]],
                       mode="lines+markers", name=t("Cost PTE (u.m.)", "PTE cost (u.m.)"),
                       line=dict(color=PINK, width=3, dash="dot"),
                       marker=dict(size=10), yaxis="y2"),
        ],
        frames=frames,
    ...
    fig_anim.update_layout(
        ...
        title=t("Simularea crizei: deficit cumulat și cost PTE (2026)",
                "Crisis simulation: cumulative deficit and PTE cost (2026)"),
        xaxis=dict(title=t("Luna", "Month"), categoryorder="array", categoryarray=months_label),
        yaxis=dict(title=t("Deficit cumulat (u.p.)", "Cumulative deficit (u.p.)"),
                   titlefont=dict(color=ORANGE), tickfont=dict(color=ORANGE),
                   range=[0, max(deficit_cum) * 1.15 + 1]),
        yaxis2=dict(title=t("Cost PTE (u.m.)", "PTE cost (u.m.)"),
                    titlefont=dict(color=PINK), tickfont=dict(color=PINK),
                    overlaying="y", side="right",
                    range=[min(cost_traj) * 0.9, max(cost_traj) * 1.1]),
        legend=dict(orientation="h", yanchor="bottom", y=-0.2, x=0.1),
        updatemenus=[dict(
            type="buttons", direction="left", x=0.05, y=1.12,
            buttons=[
                dict(label=f"▶ {t('Play', 'Play')}", method="animate",
                     args=[None, {"frame": {"duration": 700, "redraw": True},
                                   "fromcurrent": True, "transition": {"duration": 300}}]),
                dict(label=f"⏸ {t('Pauză', 'Pause')}", method="animate",
        ...
        sliders=[dict(
            steps=[dict(method="animate", args=[[str(k)],
                        ...
                        label=months_label[k - 1]) for k in range(1, 13)],
            transition=dict(duration=0), x=0.1, y=-0.15, len=0.85,
            currentvalue=dict(prefix=t("Luna: ", "Month: "), visible=True),
        )],
    )
    ...
    fig_sens.update_layout(
        ...
        yaxis=dict(title=t("Cost optim f* (u.m.)", "Optimal cost f* (u.m.)"),
                   titlefont=dict(color=PINK), tickfont=dict(color=PINK)),
        yaxis2=dict(title=t("Flux maxim v(φ) (u.p.)", "Max flow v(φ) (u.p.)"),
